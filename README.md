...
```
`await` waits for the Promise to resolve before continuing the function.
---
🪝 `useEffect()`
...
```
The empty dependency array `[]` means the effect runs after the component's initial render and does not re-run because of state or prop dependency changes.
API Fetching Flow
```text
Component Renders
       ↓
...
setData(response.data);
```
Then `.map()` can be used to display the data:
```jsx
{data.map((item) => {
  return (
    <div key={item.id}>
...
---
🔔 React Toastify
React Toastify is used to display temporary notifications in a React application.
Installation:
```bash
...
```
It provides the routing context to the components inside it.
---
🛣️ `Routes` and `Route`
...
```
`Link` allows navigation through React Router without performing a normal full-page browser navigation.
---
🧭 `useNavigate()`
`useNavigate()` allows navigation programmatically using JavaScript.
```jsx
...
Dynamic routes allow values to be part of the URL.
```jsx
<Route
  path="/products/:id"
  element={<Product />}
...
It is especially useful when many components need access to the same data.
Examples:
User information
Authentication
Theme
...
---
📤 Context Provider
The Provider supplies data to the components inside it.
Example:
```jsx
const userData = {
  username: "Captain",
...
---
👶 `children`
The `children` prop represents the components placed inside another component.
For example:
```jsx
<UserContext>
  <App />
...
```
This creates the following component structure:
```text
UserContext
     │
     ↓
...
A Context does not have to wrap the entire application. It can also be placed around only the part of the component tree that needs the data.
For example:
```text
UserContext
│
...
---
🚀 Next Steps
After completing the React fundamentals covered in the course, the next focus will be:
Strengthening React fundamentals through projects
Form Handling
